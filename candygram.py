...
def update_feed_entries(mongo_feed, entries):
    ...
    for entry in entries:
        ...
        doc['item_hash'] = get_item_hash(mongo_feed['feed'], entry)
        doc['title'] = entry['title']
        doc['url'] = entry['link']
        doc['body'] = entry['summary']
        doc['pub_ts'] = calendar.timegm(entry['published_parsed'])
        print str(doc['pub_ts'])
        id = collection.update({'item_hash' : doc['item_hash']}, doc, upsert=True)

    print collection.count()


def get_item_hash(mf, entry):
    ...
    if len(body2) < 100:
        body2 = body
    item_hash = hashlib.md5(mf['name'] + entry['title'] + body2[:100]).hexdigest()
    return item_hash
